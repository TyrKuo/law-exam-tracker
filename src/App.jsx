...
const TAG_COLORS = ["#ff6b6b","#ffa94d","#ffd43b","#69db7c","#38d9a9","#4dabf7","#748ffc","#da77f2","#f783ac","#e599f7"];
...
function getTagColor(tag){return TAG_COLORS[hashCode(tag)%TAG_COLORS.length];}
...
// 各科統計
function getSubjectStats(issues,sub){
  const subIssues=issues.filter(i=>i.subject===sub);
  const total=subIssues.length;
  if(!total)return{total:0,mastered:0,avgStage:0,avgPct:0,rememberRate:0,totalReviews:0,remembered:0};
  const mastered=subIssues.filter(i=>i.mastered).length;
  const avgStage=subIssues.reduce((s,i)=>s+i.stage,0)/total;
  const avgPct=Math.round(avgStage/6*100);
  let totalReviews=0,remembered=0;
  subIssues.forEach(i=>{const errs=(i.errors||[]).length;const reviews=i.stage+errs;totalReviews+=reviews;remembered+=i.stage;});
  const rememberRate=totalReviews>0?Math.round(remembered/totalReviews*100):0;
  return{total,mastered,avgStage:Math.round(avgStage*10)/10,avgPct,rememberRate,totalReviews,remembered};
}
function stageColor(pct){if(pct>=70)return"#3dba7a";if(pct>=40)return"#f0a840";return"#e05252";}

const s={bg:"#111318",surface:"#1a1d24",card:"#1f2330",border:"#2e3347",text:"#e8eaf0",muted:"#7b82a0",accent:"#5b6bff",accentMuted:"#1e2550",danger:"#e05252",dangerMuted:"#3d1a1a",success:"#3dba7a",successMuted:"#1a3d2d"};
...
function ColorTag({tag}){const color=getTagColor(tag);return<span className="tag" style={{background:color+"25",color:color,border:`1px solid ${color}50`,fontWeight:600}}>{tag}</span>;}
...
export default function App(){
  ...
  useEffect(()=>{load();const ch=supabase.channel("sync").on("postgres_changes",{event:"*",schema:"public",table:"issues"},load).on("postgres_changes",{event:"*",schema:"public",table:"study_log"},load).on("postgres_changes",{event:"*",schema:"public",table:"settings"},load).subscribe();sessionStart.current=Date.now();return()=>{ch.unsubscribe();const elapsed=Math.floor((Date.now()-sessionStart.current)/60000);if(elapsed>0){const today=todayStr();supabase.from("study_log").upsert({date:today,minutes:(studyLog[today]||0)+elapsed});}}},[load]);
  ...
  async function markRemember(issue){const ns=issue.stage+1,mastered=ns>=6;const next={...issue,stage:ns,lastReviewed:todayStr(),nextDate:mastered?null:calcNextDate(todayStr(),ns,issue.difficulty),mastered};setIssues(arr=>arr.map(i=>i.id===issue.id?next:i));await saveIssue(next);const rel=getRelated(issue,issues||[]);if(rel.length)setModal({type:"related_remind",related:rel});}
  function markForgot(issue){setViewIssueId(null);setTimeout(()=>setModal({type:"forgot",issue}),100);}
  async function confirmForgot(issue,reason){const newStage=Math.max(0,issue.stage-1);const next={...issue,stage:newStage,lastReviewed:todayStr(),nextDate:calcNextDate(todayStr(),newStage,issue.difficulty),mastered:false,errors:[...(issue.errors||[]),{date:todayStr(),reason}]};setIssues(arr=>arr.map(i=>i.id===issue.id?next:i));await saveIssue(next);const rel=getRelated(issue,issues||[]);setModal(rel.length?{type:"related_remind",related:rel}:null);}
  ...
  if(issues===null)return(<><style>{css}</style><div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,color:s.muted,fontSize:14,padding:20}}><div style={{fontSize:32}}>⚖️</div><div style={{color:s.text,fontWeight:600,fontSize:16}}>司法考試複習追蹤器</div><div className="spinner"/><div style={{fontSize:12}}>{syncStatus==="error"?"連線失敗，請檢查網路":"連接中…"}</div>{syncStatus==="error"&&<button onClick={load} style={{background:s.accent,color:"#fff",padding:"8px 20px",marginTop:6}}>重試</button>}</div></>);
  ...
  const allDue=prioritySort(issues.filter(isDueToday));const todayDue=allDue.slice(0,dailyLimit);const overflowCount=Math.max(0,allDue.length-dailyLimit);const todayMinutes=studyLog[todayStr()]||0;
  ...
  return(<><style>{css}</style>
    <div style={{minHeight:"100vh",background:s.bg}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",background:s.surface,borderBottom:`1px solid ${s.border}`,position:"sticky",top:0,zIndex:100,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{tabList.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{background:tab===t.id?s.accent:"transparent",color:tab===t.id?"#fff":s.muted,border:`1px solid ${tab===t.id?s.accent:s.border}`,padding:"6px 12px",borderRadius:6,fontSize:13}}>{t.label}</button>)}<button onClick={toggleSprint} style={{background:sprintMode?s.danger:"transparent",color:sprintMode?"#fff":s.muted,border:`1px solid ${sprintMode?s.danger:s.border}`,padding:"6px 12px",borderRadius:6,fontSize:13}}>{sprintMode?"衝刺中":"衝刺"}</button></div>
      </div>
      ...
      {modal?.type==="forgot"&&(<Overlay onClose={()=>setModal(null)}><div style={{padding:20}}><div style={{fontWeight:600,marginBottom:12,fontSize:15}}>標記失敗原因</div><div style={{color:s.muted,marginBottom:6,fontSize:13}}>「{modal.issue.name}」</div><div style={{color:s.muted,marginBottom:14,fontSize:11}}>將從階段 {modal.issue.stage} 退回階段 {Math.max(0,modal.issue.stage-1)}</div><div style={{display:"flex",flexDirection:"column",gap:8}}>{FAIL_REASONS.map(r=><button key={r} onClick={()=>confirmForgot(modal.issue,r)} style={{background:s.surface,color:s.text,border:`1px solid ${s.border}`,textAlign:"left",padding:"11px 14px",fontSize:14}}>{r}</button>)}</div><button onClick={()=>setModal(null)} style={{marginTop:14,background:"transparent",color:s.muted,width:"100%",border:"none",padding:"8px"}}>取消</button></div></Overlay>)}
      {modal?.type==="related_remind"&&(<Overlay onClose={()=>setModal(null)}><div style={{padding:20}}><div style={{fontWeight:600,marginBottom:10,fontSize:15}}>相關爭點提醒</div><div style={{color:s.muted,marginBottom:12,fontSize:13}}>你也記得以下關聯爭點嗎？</div>{modal.related.map(r=><div key={r.id} onClick={()=>{setModal(null);openIssueDetail(r.id);}} style={{background:s.surface,border:`1px solid ${s.border}`,borderRadius:6,padding:"9px 12px",marginBottom:8,fontSize:13,cursor:"pointer"}}><SubjectTag subject={r.subject}/> <span style={{marginLeft:4}}>{r.name}</span></div>)}<button onClick={()=>setModal(null)} style={{marginTop:12,background:s.accent,color:"#fff",width:"100%",padding:"10px"}}>了解</button></div></Overlay>)}
      {viewIssue&&!modal&&(<Overlay onClose={()=>setViewIssueId(null)}><IssueDetailModal issue={viewIssue} issues={issues} allTags={allTags} editIssue={editIssue} deleteOneIssue={deleteOneIssue} markRemember={markRemember} markForgot={markForgot} isDueToday={isDueToday} openIssueDetail={openIssueDetail} onClose={()=>setViewIssueId(null)}/></Overlay>)}
    </div></>);
}
...
function IssueDetailModal({issue,issues,allTags,editIssue,deleteOneIssue,markRemember,markForgot,isDueToday,openIssueDetail,onClose}){
  ...
  const[editName,setEditName]=useState(issue.name);
  ...
  const intervals=getIntervals(issue.difficulty);const related=getRelated(issue,issues);const isDue=isDueToday(issue);
  ...
  function startEdit(){setEditName(issue.name);setEditSubject(issue.subject);setEditDiff(issue.difficulty);setEditNotes(issue.notes||"");setEditTags(issue.tags||[]);setEditRel(issue.related||[]);setEditing(true);}
  function saveEdit(){editIssue(issue.id,{name:editName.trim()||issue.name,subject:editSubject,difficulty:editDiff,related:editRel,notes:editNotes,tags:editTags});setEditing(false);}

  return(<div style={{padding:20}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
      <div><div style={{fontWeight:700,fontSize:16,marginBottom:6}}>{issue.name}</div><div style={{display:"flex",flexWrap:"wrap",gap:4}}><SubjectTag subject={issue.subject}/><span className="tag" style={{background:"transparent",color:DIFFICULTY_COLORS[issue.difficulty],border:`1px solid ${DIFFICULTY_COLORS[issue.difficulty]}`}}>{issue.difficulty}</span>{issue.mastered&&<span className="tag" style={{background:s.successMuted,color:s.success}}>已掌握</span>}{isDue&&!issue.mastered&&<span className="tag" style={{background:s.dangerMuted,color:s.danger}}>今日到期</span>}</div></div>
      <div style={{display:"flex",gap:4}}>{!editing&&<button onClick={startEdit} style={{background:s.accentMuted,color:s.accent,fontSize:12,padding:"4px 10px"}}>編輯</button>}<button onClick={onClose} style={{background:"transparent",color:s.muted,fontSize:18,padding:"2px 6px",border:"none"}}>✕</button></div>
    </div>
    {!editing?(<>
      {(issue.tags||[]).length>0&&<div style={{marginBottom:12,display:"flex",flexWrap:"wrap",gap:4}}>{issue.tags.map(t=><ColorTag key={t} tag={t}/>)}</div>}
      <div style={{marginBottom:12}}><div style={{display:"flex",gap:3,marginBottom:4}}>{intervals.map((_,idx)=><div key={idx} style={{flex:1,height:6,borderRadius:3,background:idx<issue.stage?s.accent:s.border}}/>)}</div><div style={{fontSize:12,color:s.muted}}>階段 {Math.min(issue.stage,6)}/6{!issue.mastered&&issue.nextDate&&` · 下次：${issue.nextDate}（${dayDiff(issue.nextDate)===0?"今天":dayDiff(issue.nextDate)>0?`${dayDiff(issue.nextDate)}天後`:`逾期${-dayDiff(issue.nextDate)}天`}）`}</div></div>
      {(issue.notes||"").trim()&&(<div style={{marginBottom:12}}><div style={{fontSize:11,color:s.muted,marginBottom:4,fontWeight:600}}>筆記</div><div style={{background:s.surface,borderRadius:6,padding:"10px 12px",fontSize:13,color:s.text,lineHeight:1.7,whiteSpace:"pre-wrap"}}>{issue.notes}</div></div>)}
      {related.length>0&&(<div style={{marginBottom:12}}><div style={{fontSize:11,color:s.muted,marginBottom:4,fontWeight:600}}>關聯爭點（點擊查看）</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{related.map(r=>(<div key={r.id} onClick={()=>openIssueDetail(r.id)} style={{background:s.surface,border:`1px solid ${s.border}`,borderRadius:6,padding:"9px 12px",cursor:"pointer",fontSize:13}}><SubjectTag subject={r.subject}/> <span style={{marginLeft:4,fontWeight:500}}>{r.name}</span>{(r.notes||"").trim()&&<div style={{marginTop:4,fontSize:11,color:s.muted,lineHeight:1.5,whiteSpace:"pre-wrap",maxHeight:40,overflow:"hidden"}}>{r.notes.slice(0,80)}{r.notes.length>80?"…":""}</div>}</div>))}</div></div>)}
      {(issue.errors||[]).length>0&&(<div style={{marginBottom:12}}><div style={{fontSize:11,color:s.muted,marginBottom:4,fontWeight:600}}>錯誤紀錄</div><div style={{background:s.surface,borderRadius:6,padding:"8px 10px"}}>{issue.errors.map((e,idx)=><div key={idx} style={{fontSize:12,color:s.danger,marginBottom:2}}>{e.date} · {e.reason}</div>)}</div></div>)}
      {isDue&&!issue.mastered&&(<div style={{display:"flex",gap:8,marginBottom:12}}><button onClick={()=>{onClose();setTimeout(()=>markRemember(issue),100);}} style={{flex:1,background:s.successMuted,color:s.success,padding:"10px",fontSize:14}}>✓ 記住了</button><button onClick={()=>markForgot(issue)} style={{flex:1,background:s.dangerMuted,color:s.danger,padding:"10px",fontSize:14}}>✗ 還沒熟</button></div>)}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:11,color:s.muted}}>建立日期：{issue.created}</div>{!confirmDel?<button onClick={()=>setConfirmDel(true)} style={{background:"transparent",color:s.danger,border:`1px solid ${s.danger}`,fontSize:11,padding:"4px 10px"}}>刪除爭點</button>:<div style={{display:"flex",gap:4}}><button onClick={()=>{deleteOneIssue(issue.id);onClose();}} style={{background:s.danger,color:"#fff",fontSize:11,padding:"4px 10px"}}>確認刪除</button><button onClick={()=>setConfirmDel(false)} style={{background:"transparent",color:s.muted,border:`1px solid ${s.border}`,fontSize:11,padding:"4px 10px"}}>取消</button></div>}</div>
    </>):(<div>
      <div style={{marginBottom:10}}><Lbl>爭點名稱</Lbl><input value={editName} onChange={e=>setEditName(e.target.value)}/></div>
      <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}><div><Lbl>科目</Lbl><select value={editSubject} onChange={e=>setEditSubject(e.target.value)} style={{width:"auto"}}>{SUBJECTS.map(x=><option key={x}>{x}</option>)}</select></div><div><Lbl>難度</Lbl><select value={editDiff} onChange={e=>setEditDiff(e.target.value)} style={{width:"auto"}}>{["高","中","低"].map(d=><option key={d}>{d}</option>)}</select></div></div>
      ...
      <Lbl>關聯爭點</Lbl><SearchInput value={editRelSearch} onChange={setEditRelSearch} placeholder="搜尋爭點…"/>
...
function TagInput({tags,setTags,allTags}){
  ...
  return(<div>
    <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:6}}>{tags.map(t=><span key={t} style={{display:"inline-flex",alignItems:"center",gap:3,padding:"3px 8px",borderRadius:4,fontSize:11,fontWeight:600,background:getTagColor(t)+"25",color:getTagColor(t),border:`1px solid ${getTagColor(t)}50`}}>{t}<span onClick={()=>setTags(tags.filter(x=>x!==t))} style={{marginLeft:2,cursor:"pointer",opacity:.7}}>✕</span></span>)}</div>
    ...
    {suggestions.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:6}}>{suggestions.slice(0,8).map(t=><span key={t} onClick={()=>addTag(t)} style={{display:"inline-flex",alignItems:"center",padding:"3px 8px",borderRadius:4,fontSize:11,fontWeight:600,background:getTagColor(t)+"15",color:getTagColor(t),border:`1px solid ${getTagColor(t)}30`,cursor:"pointer"}}>+ {t}</span>)}</div>}
  </div>);
}

function SubjectStatsRow({sub,issues}){
  const st=getSubjectStats(issues,sub);
  if(!st.total)return<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><span style={{width:50,fontSize:12,color:SUBJECT_COLORS[sub],fontWeight:600,flexShrink:0}}>{sub}</span><div className="prog" style={{flex:1}}><div className="progf" style={{width:"0%"}}/></div><span style={{fontSize:11,color:s.muted,width:120,textAlign:"right"}}>0 題</span></div>;
  const color=stageColor(st.avgPct);
  return(<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
    <span style={{width:50,fontSize:12,color:SUBJECT_COLORS[sub],fontWeight:600,flexShrink:0}}>{sub}</span>
    <div className="prog" style={{flex:1}}><div className="progf" style={{width:`${st.avgPct}%`,background:color}}/></div>
    <span style={{fontSize:10,color:s.muted,width:140,textAlign:"right",flexShrink:0}}>進度{st.avgPct}% · 記住{st.rememberRate}% · {st.mastered}/{st.total}掌握</span>
  </div>);
}
...
function Dashboard({issues,todayDue,allDueCount,overflowCount,dailyLimit,updateDailyLimit,studyLog,todayMinutes,markRemember,markForgot,getDueDate,jumpToIssue,openIssueDetail}){
  const[showLimitEdit,setShowLimitEdit]=useState(false);const[tempLimit,setTempLimit]=useState(dailyLimit);
  ...
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginBottom:12}}>
      <StatBox label="今日到期" value={`${todayDue.length}/${allDueCount}`} color={s.danger}/><StatBox label="今日複習時間" value={`${todayMinutes} 分`} color={s.accent}/><StatBox label="總爭點數" value={issues.length}/><StatBox label="已掌握" value={issues.filter(i=>i.mastered).length} color={s.success}/>
    </div>
    ...
    <Sec title="各科複習進度">{SUBJECTS.map(sub=><SubjectStatsRow key={sub} sub={sub} issues={issues}/>)}</Sec>
    {Object.keys(grouped).length>0&&(<Sec title={`今日複習（${todayDue.length}）`}>{Object.entries(grouped).map(([sub,list])=>(<div key={sub} style={{marginBottom:14}}><div style={{fontSize:12,fontWeight:600,color:SUBJECT_COLORS[sub]||s.accent,marginBottom:6}}>{sub}</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{list.map(i=><IssueRow key={i.id} issue={i} onRemember={markRemember} onForgot={markForgot} openDetail={openIssueDetail}/>)}</div></div>))}</Sec>)}
    {upcoming.length>0&&(<Sec title="未來 7 天即將到期"><div style={{display:"flex",flexDirection:"column",gap:6}}>{upcoming.map(i=>{const d=dayDiff(getDueDate(i));return<div key={i.id} onClick={()=>openIssueDetail(i.id)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",background:s.surface,border:`1px solid ${s.border}`,borderRadius:6,cursor:"pointer"}}><span style={{fontSize:13}}>{i.name}</span><div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}><SubjectTag subject={i.subject}/><span style={{fontSize:11,color:s.muted}}>{d}天後</span></div></div>;})}</div></Sec>)}
  </div>);
}

function IssueRow({issue,onRemember,onForgot,openDetail}){
  return(<div style={{background:s.surface,border:`1px solid ${s.border}`,borderRadius:6}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",gap:8,flexWrap:"wrap"}}>
      <div style={{flex:1,minWidth:0,cursor:"pointer"}} onClick={()=>openDetail(issue.id)}>
        <span style={{fontSize:13,fontWeight:500}}>{issue.name}</span><span style={{marginLeft:6}}><SubjectTag subject={issue.subject}/></span><span className="tag" style={{marginLeft:3,background:"transparent",color:DIFFICULTY_COLORS[issue.difficulty],border:`1px solid ${DIFFICULTY_COLORS[issue.difficulty]}`,fontSize:10}}>{issue.difficulty}</span>
        {(issue.tags||[]).length>0&&<span style={{marginLeft:4,fontSize:10,color:getTagColor(issue.tags[0])}}>🏷{issue.tags.length}</span>}
...
function AddIssue({issues,onAdd,setTab,allTags}){
  const[name,setName]=useState("");const[subject,setSubject]=useState(SUBJECTS[0]);const[difficulty,setDifficulty]=useState("中");const[notes,setNotes]=useState("");const[tags,setTags]=useState([]);const[search,setSearch]=useState("");const[related,setRelated]=useState([]);
  ...
  return(<div style={{maxWidth:600}}><Sec title="新增爭點"><div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div><Lbl>科目</Lbl><select value={subject} onChange={e=>setSubject(e.target.value)}>{SUBJECTS.map(x=><option key={x}>{x}</option>)}</select></div><div><Lbl>難度</Lbl><select value={difficulty} onChange={e=>setDifficulty(e.target.value)}>{["高","中","低"].map(d=><option key={d}>{d}</option>)}</select></div></div>
...
function Overview({issues,markRemember,markForgot,isDueToday,editIssue,deleteIssues,deleteOneIssue,allTags,jumpSearch,setJumpSearch,jumpToIssue}){
  const[subFilter,setSubFilter]=useState("全部");const[statusFilter,setStatusFilter]=useState("全部");const[tagFilter,setTagFilter]=useState("全部");const[searchQuery,setSearchQuery]=useState(jumpSearch||"");const[sortBy,setSortBy]=useState("created_desc");const[editingId,setEditingId]=useState(null);const[selected,setSelected]=useState(new Set());const[deleteMode,setDeleteMode]=useState(false);
  ...
  if(statusFilter==="今日待複習")filtered=filtered.filter(isDueToday);else if(statusFilter==="進行中")filtered=filtered.filter(i=>!i.mastered&&!isDueToday(i));else if(statusFilter==="已掌握")filtered=filtered.filter(i=>i.mastered);
  ...
  return(<div>
    <div style={{marginBottom:12}}><SearchInput value={searchQuery} onChange={setSearchQuery} placeholder="🔍 搜尋爭點名稱、筆記、標籤…"/>
...
function IssueCard({issue,issues,isDue,onRemember,onForgot,editing,setEditing,editIssue,deleteMode,selected,onToggleSelect,onDelete,allTags,jumpToIssue}){
  const[editName,setEditName]=useState(issue.name);const[editSubject,setEditSubject]=useState(issue.subject);const[editDiff,setEditDiff]=useState(issue.difficulty);const[editNotes,setEditNotes]=useState(issue.notes||"");const[editTags,setEditTags]=useState(issue.tags||[]);const[editRelSearch,setEditRelSearch]=useState("");const[editRel,setEditRel]=useState(issue.related||[]);const[showNotes,setShowNotes]=useState(false);const[confirmDel,setConfirmDel]=useState(false);
  const intervals=getIntervals(issue.difficulty);const related=getRelated(issue,issues);
  const relSearch=editRelSearch.length>=1?issues.filter(i=>(i.name.includes(editRelSearch)||i.subject.includes(editRelSearch))&&i.id!==issue.id&&!editRel.includes(i.id)):[];
  function saveEdit(){editIssue(issue.id,{name:editName.trim()||issue.name,subject:editSubject,difficulty:editDiff,related:editRel,notes:editNotes,tags:editTags});setEditing(null);}
  function startEdit(){setEditName(issue.name);setEditSubject(issue.subject);setEditDiff(issue.difficulty);setEditNotes(issue.notes||"");setEditTags(issue.tags||[]);setEditRel(issue.related||[]);setEditing(issue.id);}

  return(<div style={{background:selected?s.dangerMuted:s.card,border:`1px solid ${selected?s.danger:isDue?s.danger:s.border}`,borderRadius:8,padding:"13px 15px",transition:"background .15s"}}>
    <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:6,marginBottom:9}}>
          <div><span style={{fontWeight:600,fontSize:14}}>{issue.name}</span><span style={{marginLeft:6}}><SubjectTag subject={issue.subject}/></span><span className="tag" style={{marginLeft:3,background:"transparent",color:DIFFICULTY_COLORS[issue.difficulty],border:`1px solid ${DIFFICULTY_COLORS[issue.difficulty]}`,fontSize:10}}>{issue.difficulty}</span>{issue.mastered&&<span className="tag" style={{marginLeft:3,background:s.successMuted,color:s.success}}>已掌握</span>}{isDue&&!issue.mastered&&<span className="tag" style={{marginLeft:3,background:s.dangerMuted,color:s.danger}}>今日到期</span>}</div>
        ...
        {(issue.tags||[]).length>0&&<div style={{marginBottom:7,display:"flex",flexWrap:"wrap",gap:4}}>{issue.tags.map(t=><ColorTag key={t} tag={t}/>)}</div>}
        <div style={{marginBottom:9}}><div style={{display:"flex",gap:3,marginBottom:4}}>{intervals.map((_,idx)=><div key={idx} style={{flex:1,height:5,borderRadius:3,background:idx<issue.stage?s.accent:s.border}}/>)}</div><div style={{fontSize:11,color:s.muted}}>階段 {Math.min(issue.stage,6)}/6{!issue.mastered&&issue.nextDate&&` · 下次：${issue.nextDate}（${dayDiff(issue.nextDate)===0?"今天":dayDiff(issue.nextDate)>0?`${dayDiff(issue.nextDate)}天後`:`逾期${-dayDiff(issue.nextDate)}天`}）`}</div></div>
        ...
        {related.length>0&&<div style={{marginBottom:7}}><span style={{fontSize:11,color:s.muted}}>關聯：</span>{related.map(r=><span key={r.id} className="tag" style={{marginLeft:4,background:s.accentMuted,color:s.accent,fontSize:11,cursor:"pointer"}}>{r.name}</span>)}</div>}
        ...
        {editing&&(<div style={{borderTop:`1px solid ${s.border}`,paddingTop:12,marginBottom:8}}>
          <div style={{marginBottom:10}}><Lbl>爭點名稱</Lbl><input value={editName} onChange={e=>setEditName(e.target.value)}/></div>
...
function Stats({issues,studyLog}){
  ...
  const ec={};FAIL_REASONS.forEach(r=>ec[r]=0);issues.forEach(i=>(i.errors||[]).forEach(e=>{if(ec[e.reason]!==undefined)ec[e.reason]++;}));const maxErr=Math.max(...Object.values(ec),1);
