...
const RATINGS=[
  ...
  {id:"hard",label:"勉強記得",color:"#f0a840",delta:-1,icon:"😓"},
  {id:"good",label:"記住了",color:"#3dba7a",delta:1,icon:"👍"},
  {id:"easy",label:"很熟",color:"#5b6bff",delta:2,icon:"🔥"},
];
...
const dd=ds=>Math.round((new Date(ds)-new Date(td()))/864e5);
...
function pSort(l){const d={高:0,中:1,低:2};return[...l].sort((a,b)=>{const ad=-dd(a.nextDate||td()),bd=-dd(b.nextDate||td());if(ad!==bd)return bd-ad;if((d[a.difficulty]??1)!==(d[b.difficulty]??1))return(d[a.difficulty]??1)-(d[b.difficulty]??1);if((b.errors||[]).length!==(a.errors||[]).length)return(b.errors||[]).length-(a.errors||[]).length;return a.stage-b.stage;});}
...
function renderNotes(text,issues,onClick){
  ...
  for(let i=0;i<lines.length;i++){
    const line=lines[i];
    // 計算縮排層級：每2個空格或1個tab算一層
    const match=line.match(/^(\s*)(- |• |· |‣ |‐ )?(.*)$/);
    if(!match){result.push(mkLine(line,0,i,issues,onClick));continue;}
    const spaces=match[1]||"";
    const bullet=match[2]||"";
    const content=match[3]||"";
    if(bullet){
      const indent=Math.floor(spaces.length/2)+1;
      result.push(mkLine(content,indent,i,issues,onClick));
    }else if(spaces.length>0&&lines.some((l,j)=>j<i&&l.match(/^\s*[-•·‣‐] /))){
      // 連續文字屬於上一個項目的子內容
      const indent=Math.floor(spaces.length/2)+1;
      result.push(mkLine(content,indent,i,issues,onClick));
    }else{
      result.push(mkLine(line,0,i,issues,onClick));
    }
  }
  return result;
}

function mkLine(text,indent,key,issues,onClick){
  // 解析 [[連結]]
  const parts=text.split(/(\[\[[^\]]+\]\])/g).map((p,i)=>{
    const m=p.match(/^\[\[([^\]]+)\]\]$/);
    if(m){
      const name=m[1];
      const found=issues.find(x=>x.name===name);
      return <span key={i} onClick={e=>{e.stopPropagation();found&&onClick?.(found.id);}} style={{color:"#5b6bff",cursor:found?"pointer":"default",textDecoration:"underline",textDecorationStyle:"dotted",textUnderlineOffset:2}}>{name}{!found&&<span style={{color:"#e05252",fontSize:9,marginLeft:2}}>?</span>}</span>;
    }
    // 解析 **粗體** 和 *斜體*
    const styled=p.split(/(\*\*[^*]+\*\*|\*[^*]+\*)/).map((s,j)=>{
      if(s.startsWith("**")&&s.endsWith("**"))return<strong key={j} style={{color:"#e8eaf0"}}>{s.slice(2,-2)}</strong>;
      if(s.startsWith("*")&&s.endsWith("*"))return<em key={j} style={{color:"#b8a0ff"}}>{s.slice(1,-1)}</em>;
      return<span key={j}>{s}</span>;
    });
    return<span key={i}>{styled}</span>;
  });

  const colors=["#7b82a0","#5b6bff","#3dba7a","#f0a840","#da77f2","#38d9a9"];
  const bulletColor=colors[Math.min(indent,colors.length)-1]||colors[0];

  return<div key={key} style={{paddingLeft:indent*20,marginBottom:3,fontSize:13,color:"#e8eaf0",lineHeight:1.8,display:"flex",alignItems:"flex-start"}}>
    {indent>0&&<span style={{color:bulletColor,marginRight:8,flexShrink:0,fontSize:indent===1?13:11,lineHeight:"1.8"}}>{indent===1?"•":indent===2?"◦":"‣"}</span>}
...
textarea.nl{min-height:160px;font-size:14px;line-height:1.8;resize:vertical;font-family:'SF Mono',Menlo,Monaco,monospace}
...
const CT=({t})=>{const c=tc(t);return<span className="tag" style={{background:c+"25",color:c,border:`1px solid ${c}50`,fontWeight:600}}>{t}</span>;};
...
const RB=({onRate,sm})=>{const p=sm?"8px 2px":"12px 4px";const fs=sm?14:20;const ls=sm?9:11;
  return<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:sm?6:8}}>{RATINGS.map(r=><button key={r.id} onClick={()=>onRate(r)} style={{background:r.color+"20",color:r.color,border:`1px solid ${r.color}40`,padding:p,borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",gap:sm?2:4}}><span style={{fontSize:fs}}>{r.icon}</span><span style={{fontSize:ls,fontWeight:600}}>{r.label}</span>{!sm&&<span style={{fontSize:9,color:C.mt}}>{r.delta>0?"+":""}{r.delta}階</span>}</button>)}</div>;};
...
function EP({issue,issues,allTags,editIssue,onDone}){
  ...
  return<div style={{borderTop:`1px solid ${C.bd}`,paddingTop:12}}>
    ...
    <div style={{marginBottom:10}}><Lb ch={<>筆記 <span style={{fontWeight:400,fontSize:11}}>（- 階層 · [[連結]] · **粗體** · *斜體*）</span></>}/><textarea className="nl" value={nt} onChange={e=>sNt(e.target.value)} placeholder={NOTE_PH}/></div>
...
export default function App(){
  ...
  const[fc,_setFc]=useState(false);
  const fcR=useRef(false);
  const setFc=v=>{fcR.current=v;_setFc(v);};
  ...
  const load=useCallback(async()=>{
    try{setSync("loading");
      ...
      setIssues((iR.data||[]).map(db2i));
      const l={};(lR.data||[]).forEach(r=>{l[r.date]=r.minutes;});setLog(l);
      setSprint(sR.data?.value===true);if(dR.data?.value)setLimit(dR.data.value);setSync("synced");
  ...
  useEffect(()=>{
    ...
    const ch=supabase.channel("sync")
      .on("postgres_changes",{event:"*",schema:"public",table:"issues"},()=>{if(!fcR.current)load();})
      .on("postgres_changes",{event:"*",schema:"public",table:"study_log"},()=>{if(!fcR.current)load();})
      .on("postgres_changes",{event:"*",schema:"public",table:"settings"},()=>{if(!fcR.current)load();})
      .subscribe();
    ss.current=Date.now();
    return()=>{ch.unsubscribe();const el=Math.floor((Date.now()-ss.current)/60000);if(el>0)supabase.from("study_log").upsert({date:td(),minutes:(log[td()]||0)+el});};
  },[load]);
  ...
  async function rate(iss,r){const ns=Math.max(0,Math.min(6,iss.stage+r.delta));const m=ns>=6;const errs=r.fail?[...(iss.errors||[]),{date:td(),reason:r.fail}]:(iss.errors||[]);const next={...iss,stage:ns,lastReviewed:td(),nextDate:m?null:cNext(td(),ns,iss.difficulty),mastered:m,errors:errs};setIssues(a=>a.map(i=>i.id===iss.id?next:i));await sv(next);return next;}
  async function addI(iss){const n={...iss,id:mid(),created:td(),stage:0,nextDate:cNext(td(),0,iss.difficulty),lastReviewed:null,mastered:false,errors:[],related:iss.related||[]};setIssues(a=>[...a,n]);await sv(n);}
  ...
  const scl=sync==="synced"?C.ok:sync==="error"?C.dg:"#f0a840";
  ...
  return<><style>{css}</style>
    <div style={{minHeight:"100vh",background:C.bg}}>
      ...
      <div style={{padding:16,maxWidth:900,margin:"0 auto"}}>
        ...
        {tab==="add"&&<Add issues={issues} onAdd={addI} setTab={setTab} allTags={allTags} draft={draft} setDraft={setDraft}/>}
        {tab==="overview"&&<OvW issues={issues} rate={rate} isDue={isDue} editI={editI} delM={delM} del1={del1} allTags={allTags} js={js} setJs={setJs}/>}
        {tab==="stats"&&<Stats issues={issues} log={log}/>}
      </div>
      {modal?.type==="related"&&<Ov ch={<div style={{padding:20}}><div style={{fontWeight:600,marginBottom:10,fontSize:15}}>相關爭點提醒</div>{modal.related.map(r=><div key={r.id} onClick={()=>{setModal(null);openD(r.id);}} style={{background:C.sf,border:`1px solid ${C.bd}`,borderRadius:6,padding:"9px 12px",marginBottom:8,fontSize:13,cursor:"pointer"}}><ST s={r.subject}/> <span style={{marginLeft:4}}>{r.name}</span></div>)}<button onClick={()=>setModal(null)} style={{marginTop:12,background:C.ac,color:"#fff",width:"100%",padding:10}}>了解</button></div>} oc={()=>setModal(null)}/>}
...
function FCM({issues,queue,rate,exit}){
  ...
  async function go(r){await rate(cur,r);setRes(p=>[...p,{issue:cur,rating:r}]);setFl(false);if(idx+1>=queue.length)setDone(true);else setIdx(idx+1);}
  ...
  if(done){const ct={};RATINGS.forEach(r=>ct[r.id]=0);res.forEach(r=>ct[r.rating.id]++);
    return<div style={{minHeight:"100vh",background:C.bg,padding:20,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
      <div style={{fontSize:40}}>📊</div><div style={{fontSize:18,fontWeight:600}}>複習完成！共 {res.length} 張</div>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,maxWidth:600,margin:"0 auto",width:"100%"}}>
      ...
      :<div className="fi" style={{width:"100%"}}>
        <div style={{background:C.sf,border:`1px solid ${C.bd}`,borderRadius:12,padding:20,marginBottom:20,maxHeight:"40vh",overflowY:"auto"}}>
          {(cur.notes||"").trim()?<div>{renderNotes(cur.notes,issues,null)}</div>:<div style={{color:C.mt,textAlign:"center",padding:16,fontSize:13}}>（尚無筆記）</div>}
...
function Det({issue:is,issues,allTags,editI,del1,rate,isDue,openD,oc}){
  ...
  return<div style={{padding:20}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
      <div><div style={{fontWeight:700,fontSize:16,marginBottom:6}}>{is.name}</div><div style={{display:"flex",flexWrap:"wrap",gap:4}}><ST s={is.subject}/><span className="tag" style={{background:"transparent",color:DC[is.difficulty],border:`1px solid ${DC[is.difficulty]}`}}>{is.difficulty}</span>{is.mastered&&<span className="tag" style={{background:C.om,color:C.ok}}>已掌握</span>}{due&&!is.mastered&&<span className="tag" style={{background:C.dm,color:C.dg}}>今日到期</span>}</div></div>
      <div style={{display:"flex",gap:4}}>{!ed&&<button onClick={()=>sEd(true)} style={{background:C.am,color:C.ac,fontSize:12,padding:"4px 10px"}}>編輯</button>}<button onClick={oc} style={{background:"transparent",color:C.mt,fontSize:18,padding:"2px 6px",border:"none"}}>✕</button></div>
    </div>
    {!ed?<>
      {(is.tags||[]).length>0&&<div style={{marginBottom:12,display:"flex",flexWrap:"wrap",gap:4}}>{is.tags.map(t=><CT key={t} t={t}/>)}</div>}
      <div style={{marginBottom:12}}><div style={{display:"flex",gap:3,marginBottom:4}}>{itv.map((_,i)=><div key={i} style={{flex:1,height:6,borderRadius:3,background:i<is.stage?C.ac:C.bd}}/>)}</div><div style={{fontSize:12,color:C.mt}}>階段 {Math.min(is.stage,6)}/6{!is.mastered&&is.nextDate&&` · 下次：${is.nextDate}（${dd(is.nextDate)===0?"今天":dd(is.nextDate)>0?`${dd(is.nextDate)}天後`:`逾期${-dd(is.nextDate)}天`}）`}</div></div>
      {(is.notes||"").trim()&&<div style={{marginBottom:12}}><Lb ch={<>筆記 <span style={{fontWeight:400,fontSize:10}}>（[[名稱]] 可點擊）</span></>}/><div style={{background:C.sf,borderRadius:6,padding:"12px 14px"}}>{renderNotes(is.notes,issues,openD)}</div></div>}
...
function Dash({issues,due,allN,ovf,limit,setLim,mins,rate,gDue,openD,startFC}){
  ...
  const up=issues.filter(i=>!i.mastered&&dd(gDue(i))>0&&dd(gDue(i))<=7).sort((a,b)=>gDue(a).localeCompare(gDue(b)));
  return<div>
    ...
    {up.length>0&&<Sec t="未來 7 天到期" ch={<div style={{display:"flex",flexDirection:"column",gap:6}}>{up.map(i=>{const d=dd(gDue(i));return<div key={i.id} onClick={()=>openD(i.id)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",background:C.sf,border:`1px solid ${C.bd}`,borderRadius:6,cursor:"pointer"}}><span style={{fontSize:13}}>{i.name}</span><div style={{display:"flex",gap:6,alignItems:"center"}}><ST s={i.subject}/><span style={{fontSize:11,color:C.mt}}>{d}天後</span></div></div>;})}</div>}/>}
  </div>;
}
...
function Row({i,rate,openD}){const[sr,sSr]=useState(false);
  return<div style={{background:C.sf,border:`1px solid ${C.bd}`,borderRadius:6}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",gap:8,flexWrap:"wrap"}}>
      <div style={{flex:1,minWidth:0,cursor:"pointer"}} onClick={()=>openD(i.id)}><span style={{fontSize:13,fontWeight:500}}>{i.name}</span><span style={{marginLeft:6}}><ST s={i.subject}/></span><span className="tag" style={{marginLeft:3,background:"transparent",color:DC[i.difficulty],border:`1px solid ${DC[i.difficulty]}`,fontSize:10}}>{i.difficulty}</span>{(i.tags||[]).length>0&&<span style={{marginLeft:4,fontSize:10,color:tc(i.tags[0])}}>🏷{i.tags.length}</span>}{(i.notes||"").trim()&&<span style={{marginLeft:4,fontSize:10,color:C.mt}}>📝</span>}</div>
      <button onClick={e=>{e.stopPropagation();sSr(!sr);}} style={{background:C.am,color:C.ac,fontSize:11,padding:"5px 10px",flexShrink:0}}>{sr?"收起":"評分"}</button>
    </div>
    {sr&&<div style={{padding:"0 12px 10px"}}><RB onRate={async r=>{await rate(i,r);sSr(false);}} sm/></div>}
  </div>;
}
...
function Add({issues,onAdd,setTab,allTags,draft,setDraft}){
  ...
  return<div style={{maxWidth:600}}><Sec t="新增爭點" ch={<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div><Lb ch={<>筆記 <span style={{fontWeight:400,fontSize:11}}>（- 階層 · [[連結]] · **粗體** · *斜體*）</span></>}/><textarea className="nl" value={notes} onChange={e=>up("notes",e.target.value)} placeholder={NOTE_PH}/></div>
...
function OvW({issues,rate,isDue,editI,delM,del1,allTags,js,setJs}){
  ...
  let f=issues;if(sf!=="全部")f=f.filter(i=>i.subject===sf);
  ...
  const sA=()=>sel.size===f.length?sSel(new Set()):sSel(new Set(f.map(i=>i.id)));
  const cD=()=>{delM([...sel]);sSel(new Set());sDm(false);};
  return<div>
    <div style={{marginBottom:12}}><SI v={sq} oc={sSq} p="🔍 搜尋爭點名稱、筆記、標籤…"/>
      <div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"center",marginTop:10}}>
        ...
        <div style={{marginLeft:"auto"}}>{!dm?<button onClick={()=>sDm(true)} style={{background:"transparent",color:C.dg,border:`1px solid ${C.dg}`,fontSize:12,padding:"6px 10px"}}>批量刪除</button>:<div style={{display:"flex",gap:6,alignItems:"center"}}><button onClick={sA} style={{background:"transparent",color:C.mt,border:`1px solid ${C.bd}`,fontSize:12,padding:"6px 9px"}}>{sel.size===f.length?"取消全選":"全選"}</button><button onClick={cD} disabled={!sel.size} style={{background:C.dg,color:"#fff",fontSize:12,padding:"6px 10px"}}>刪除{sel.size?` (${sel.size})`:""}</button><button onClick={()=>{sSel(new Set());sDm(false);}} style={{background:"transparent",color:C.mt,border:`1px solid ${C.bd}`,fontSize:12,padding:"6px 9px"}}>取消</button></div>}</div>
      </div>
    </div>
