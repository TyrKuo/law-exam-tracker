...
function renderNotes(text,issues,onClick){
  ...
  return text.split("\n").map((line,i)=>{
    ...
    if(!m)return mkLine(line,0,i,issues,onClick);
    const sp=m[1]||"",bullet=m[2]||"",content=m[3]||"";
    if(bullet){const ind=Math.floor(sp.length/2)+1;return mkLine(content,ind,i,issues,onClick);}
    return mkLine(line,0,i,issues,onClick);
  });
}
function mkLine(text,ind,key,issues,onClick){
  ...
  const colors=["#7b82a0","#5b6bff","#3dba7a","#f0a840","#da77f2"];
  const bc=colors[Math.min(ind,colors.length)-1]||colors[0];
  const bullets=["•","◦","‣","·"];
  return<div key={key} style={{paddingLeft:ind*20,marginBottom:3,fontSize:13,color:"#e8eaf0",lineHeight:1.8,display:"flex",alignItems:"flex-start"}}>{ind>0&&<span style={{color:bc,marginRight:8,flexShrink:0}}>{bullets[Math.min(ind-1,bullets.length-1)]}</span>}<span style={{flex:1}}>{parts}</span></div>;
}
...
textarea.nl{min-height:160px;font-size:14px;line-height:1.8;resize:vertical}
@keyframes fi{from{opacity:0;transform:rotateX(-10deg)}to{opacity:1;transform:rotateX(0)}}.fi{animation:fi .3s ease}
.tb{display:flex;gap:4px;padding:6px 8px;background:${C.sf};border:1px solid ${C.bd};border-bottom:none;border-radius:6px 6px 0 0;flex-wrap:wrap}
.tb button{padding:4px 10px;font-size:12px;background:${C.bg};color:${C.mt};border:1px solid ${C.bd};border-radius:4px}
.tb button:hover{color:${C.ac};border-color:${C.ac}}
.nl-wrap textarea{border-radius:0 0 6px 6px;border-top:none}
`;
...
// ═══ 筆記編輯器（含格式工具列）═══
function NE({value,onChange,issues,placeholder}){
  ...
  function insertAt(before,after=""){
    ...
    onChange(newText);
    setTimeout(()=>{ta.focus();const pos=start+before.length+(sel?sel.length:0);ta.setSelectionRange(sel?start+before.length+sel.length+after.length:pos,sel?start+before.length+sel.length+after.length:pos);},10);
  }
  ...
  function insertLink(name){
    insertAt(`[[${name}]]`);
    setShowLink(false);setLinkSearch("");
  }

  const linkResults=linkSearch.length>=1?(issues||[]).filter(i=>i.name.includes(linkSearch)).slice(0,6):[];

  return<div>
    <div className="tb">
      <button onClick={addBullet} title="新增項目">• 項目</button>
      <button onClick={addIndent} title="增加縮排">↳ 縮排</button>
      <button onClick={()=>setShowLink(!showLink)} title="連結爭點" style={showLink?{color:C.ac,borderColor:C.ac}:{}}>🔗 連結</button>
      <button onClick={()=>insertAt("**","**")} title="粗體"><strong>B</strong></button>
      <button onClick={()=>insertAt("*","*")} title="斜體"><em>I</em></button>
    </div>
    {showLink&&<div style={{background:C.sf,border:`1px solid ${C.bd}`,borderBottom:"none",padding:"8px"}}>
      <input value={linkSearch} onChange={e=>setLinkSearch(e.target.value)} placeholder="搜尋爭點名稱…" style={{fontSize:13,padding:"6px 8px"}} autoFocus/>
      {linkResults.length>0&&<div style={{marginTop:4,maxHeight:120,overflowY:"auto"}}>{linkResults.map(i=><div key={i.id} onClick={()=>insertLink(i.name)} style={{padding:"6px 8px",cursor:"pointer",fontSize:12,borderBottom:`1px solid ${C.bd}`}}><ST s={i.subject}/> <span style={{marginLeft:4}}>{i.name}</span></div>)}</div>}
      {linkSearch&&!linkResults.length&&<div style={{fontSize:11,color:C.mt,padding:"6px 0"}}>找不到「{linkSearch}」，你可以直接手打 [[{linkSearch}]]</div>}
    </div>}
    <div className="nl-wrap"><textarea ref={ref} className="nl" value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder||"輸入筆記…"} onKeyDown={e=>{
      if(e.key==="Enter"){
        const ta=e.target;const pos=ta.selectionStart;const lineStart=value.lastIndexOf("\n",pos-1)+1;const line=value.substring(lineStart,pos);
        const bm=line.match(/^(\s*)([-•◦‣] )/);
        if(bm){e.preventDefault();const indent=bm[1]+bm[2];const newText=value.substring(0,pos)+"\n"+indent+value.substring(pos);onChange(newText);setTimeout(()=>{ta.focus();const np=pos+1+indent.length;ta.setSelectionRange(np,np);},10);}
      }
      if(e.key==="Tab"){e.preventDefault();addIndent();}
    }}/></div>
  </div>;
}
...
function FCM({issues,queue,rate,exit}){
  ...
  if(done){const ct={};RATINGS.forEach(r=>ct[r.id]=0);res.forEach(r=>ct[r.rating.id]++);return<div style={{minHeight:"100vh",background:C.bg,padding:20,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}><div style={{fontSize:40}}>📊</div><div style={{fontSize:18,fontWeight:600}}>複習完成！共 {res.length} 張</div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,width:"100%",maxWidth:400}}>{RATINGS.map(r=><div key={r.id} style={{background:C.sf,border:`1px solid ${C.bd}`,borderRadius:8,padding:12,textAlign:"center"}}><div style={{fontSize:20}}>{r.icon}</div><div style={{fontSize:11,color:r.color,fontWeight:600,marginTop:4}}>{r.label}</div><div style={{fontSize:22,fontWeight:700,marginTop:4}}>{ct[r.id]}</div></div>)}</div><button onClick={exit} style={{background:C.ac,color:"#fff",padding:"10px 24px",fontSize:14,marginTop:8}}>返回首頁</button></div>;}
...
function Det({issue:is,issues,allTags,editI,del1,rate,isDue,openD,oc}){
  ...
  return<div style={{padding:20}}>
    ...
    {!ed?<>
      ...
      {(is.notes||"").trim()&&<div style={{marginBottom:12}}><Lb ch="筆記"/><div style={{background:C.sf,borderRadius:6,padding:"12px 14px"}}>{renderNotes(is.notes,issues,openD)}</div></div>}
